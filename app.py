...
<div class="context-box">
    ...
    <p>
        ...
        buyers make smarter decisions, helps cities identify undervalued neighborhoods, and is a foundational
...
with tab1:
    ...
    with col2:
        st.markdown(f"""
        <div class="prediction-box" style="border-color: #6b9ff0; background: linear-gradient(135deg, #1a2a3f, #1f2a3a);">
    ...
    with c1:
        st.markdown(f'<div class="metric-card"><h2>{models["rf_r2"]:.3f}</h2><p>Random Forest R²</p></div>', unsafe_allow_html=True)
    with c2:
        st.markdown(f'<div class="metric-card"><h2>${models["rf_rmse"]*100000:,.0f}</h2><p>Random Forest RMSE</p></div>', unsafe_allow_html=True)
    with c3:
        st.markdown(f'<div class="metric-card"><h2>{models["lr_r2"]:.3f}</h2><p>Linear Regression R²</p></div>', unsafe_allow_html=True)
    with c4:
...
with tab2:
    ...
    st.caption("Each dot is a census block. Size = median income. Color = house price. Coastal areas command premium prices.")
